<template><div><Toc /><h1 id="redis-之hyperloglog" tabindex="-1"><a class="header-anchor" href="#redis-之hyperloglog"><span>Redis 之HyperLogLog</span></a></h1>
<p>Redis在2.8.9版本中添加了hyperLogLog结构（估算）
它是用来做基数统计的结构。
<strong>优点：</strong>
输入元素非常大的时候，计算基数所需要的空间是固定且很小的（当存储的元素达到某个阈值后，需要花12kb来存这个键，可以存2^64个不同元素）
<strong>缺点：</strong>
hyperLogLog结构只能记录有多少个不同的元素，无法像集合一样拿出元素使用</p>
<div class="language-shell line-numbers-mode" data-highlighter="shiki" data-ext="shell" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code class="language-shell"><span class="line"><span style="--shiki-light:#4078F2;--shiki-dark:#61AFEF">pfadd</span><span style="--shiki-light:#50A14F;--shiki-dark:#98C379"> key</span><span style="--shiki-light:#50A14F;--shiki-dark:#98C379"> element</span><span style="--shiki-light:#383A42;--shiki-dark:#ABB2BF"> [element...]  // 添加指定元素到key</span></span>
...
// todo</p>
<h2 id="hyperloglog相关命令详解" tabindex="-1"><a class="header-anchor" href="#hyperloglog相关命令详解"><span>HyperLogLog相关命令详解：</span></a></h2>
<p>pfadd hll ele：将ele添加进hll的基数计算中。流程：
...
pfmerge hll3 hll1 hll2：将hll1和hll2合并成hll3。合并方法是将所有的HyperLogLog对象合并到一个名为max的对象中，max采用的  是密集存储结构，如果被合并的对象也是密集存储结构，则循环比较每一个计数值，将大的那个存入max。</p>
<p>Redis的所有HyperLogLog结构都是固定的16384个桶（2的14次方），并且有两种存储格式：</p>
<p>稀疏格式：HyperLogLog算法在刚开始的时候，大多数桶其实都是0，稀疏格式通过存储连续的0的数目，而不是每个0存一遍，大大减小了HyperLogLog刚开始时需要占用的内存</p>
<p>紧凑格式：用6个bit表示一个桶，需要占用12KB内存</p>
<h2 id="pf-的内存占用为什么是-12k" tabindex="-1"><a class="header-anchor" href="#pf-的内存占用为什么是-12k"><span>pf 的内存占用为什么是 12k？</span></a></h2>
<p>我们在上面的算法中使用了1024个桶进行独立计数，不过在Redis的HyperLogLog实现中用到的是16384个桶，也就是2^14，每个桶的maxbits需要6个bits来存储，最大可以表示maxbits=63，于是总共占用内存就是2^14 * 6/8 = 12k 字节。</p>
</div></template>
