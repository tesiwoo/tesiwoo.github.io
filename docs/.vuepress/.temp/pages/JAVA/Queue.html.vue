<template><div><h1 id="java-队列-queue" tabindex="-1"><a class="header-anchor" href="#java-队列-queue"><span>JAVA-队列（Queue）</span></a></h1>
<p>Queue：基本上，一个队列就是一个先进先出（FIFO）的数据结构
Queue继承了Collection接口，是和List，Set同级别的</p>
<h2 id="queue的实现" tabindex="-1"><a class="header-anchor" href="#queue的实现"><span>Queue的实现</span></a></h2>
<h3 id="没有实现阻塞接口-非阻塞-实现了java-util-queue接口和java-util-abstractqueue接口" tabindex="-1"><a class="header-anchor" href="#没有实现阻塞接口-非阻塞-实现了java-util-queue接口和java-util-abstractqueue接口"><span>没有实现阻塞接口（非阻塞） 实现了java.util.Queue接口和java.util.AbstractQueue接口</span></a></h3>
<ul>
<li>
<h2 id="内置的不阻塞队列-priorityqueue-和-concurrentlinkedqueue" tabindex="-1"><a class="header-anchor" href="#内置的不阻塞队列-priorityqueue-和-concurrentlinkedqueue"><span>内置的不阻塞队列：<strong>PriorityQueue</strong> 和 <strong>ConcurrentLinkedQueue</strong></span></a></h2>
</li>
</ul>
<h3 id="实现了阻塞接口的-bloclingqueue" tabindex="-1"><a class="header-anchor" href="#实现了阻塞接口的-bloclingqueue"><span>实现了阻塞接口的 (BloclingQueue)</span></a></h3>
<ul>
...
<td>add</td>
<td>往队列里新增一个元素</td>
<td>IIIegaISlabEepeplian异常</td>
</tr>
<tr>
<td>remove</td>
<td>移出队列头部的元素（返回头部元素并删除）</td>
<td>NoSuchElementException异常</td>
</tr>
<tr>
<td>element</td>
<td>返回队列头部的元素（不删除操作）</td>
<td>NoSuchElementException异常</td>
</tr>
<tr>
<td>offer</td>
<td>往队列里新增一个元素并返回true（已满返回false）</td>
<td>返回false</td>
</tr>
<tr>
<td>poll</td>
...
<tr>
<td>**remove、element、offer 、poll、peek 属于Queue接口。 **</td>
<td></td>
<td></td>
</tr>
</tbody>
</table>
<p>阻塞队列的操作可以根据它们的响应方式分为以下三类：aad、removee和element操作在你试图为一个已满的队列增加元素或从空队列取得元素时 抛出异常。在多线程程序中，队列在任何时间都可能变成满的或空的， 一般使用offer、poll、peek方法（因为它不会抛出异常）。
注意：队列中插入null值是不合法
最后，我们有阻塞操作put和take。put方法在队列满时阻塞，take方法在队列空时阻塞。</p>
<ul>
<li>LinkedBlockingQueue在不指定时容量时它的长度为Integer.MAX_VALUE，，但是也可以选择指定其最大容量，它是基于链表的队列，此队列按 FIFO（先进先出）排序元素</li>
<li>ArrayBlockingQueue在构造时需要指定容量， 并可以选择是否需要公平性，如果公平参数被设置true，等待时间最长的线程会优先得到处理（其实就是通过将ReentrantLock设置为true来 达到这种公平性的：即等待时间最长的线程会先操作）。通常，公平性会使你在性能上付出代价，只有在的确非常需要的时候再使用它。它是基于数组的阻塞循环队 列，此队列按 FIFO（先进先出）原则对元素进行排序。</li>
<li>PriorityBlockingQueue是一个带优先级的队列，而不是先进先出队列。元素按优先级顺序被移除，该队列也没有上限（看了一下源码，PriorityBlockingQueue是对 PriorityQueue的再次包装，是基于堆数据结构的，而PriorityQueue是没有容量限制的，与ArrayList一样，所以在优先阻塞 队列上put时是不会受阻的。虽然此队列逻辑上是无界的，但是由于资源被耗尽，所以试图执行添加操作可能会导致 OutOfMemoryError），但是如果队列为空，那么取元素的操作take就会阻塞，所以它的检索操作take是受阻的。另外，往入该队列中的元 素要具有比较能力。</li>
