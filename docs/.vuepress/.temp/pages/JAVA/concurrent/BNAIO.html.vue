<template><div><p>Java 中的 BIO、NIO和 AIO 理解为是 Java 语言对操作系统的各种 IO 模型的封装。程序员在使用这些 API 的时候，不需要关心操作系统层面的知识，也不需要根据不同操作系统编写不同的代码。只需要使用Java的API就可以了。</p>
<h2 id="同步与异步-阻塞与非阻塞。" tabindex="-1"><a class="header-anchor" href="#同步与异步-阻塞与非阻塞。"><span>同步与异步，阻塞与非阻塞。</span></a></h2>
<ul>
<li><strong>同步</strong> ：两个同步任务相互依赖，并且一个任务必须以依赖于另一任务的某种方式执行。 比如在<code v-pre>A-&gt;B</code>事件模型中，你需要先完成 A 才能执行B。 再换句话说，同步调用中被调用者未处理完请求之前，调用不返回，调用者会一直等待结果的返回。</li>
...
<h2 id="bio" tabindex="-1"><a class="header-anchor" href="#bio"><span>BIO</span></a></h2>
...
<h3 id="伪异步io" tabindex="-1"><a class="header-anchor" href="#伪异步io"><span>伪异步IO</span></a></h3>
...
<p><strong>BIO例题见Basic-Code/liework/src/com/IO</strong></p>
...
<h2 id="nio" tabindex="-1"><a class="header-anchor" href="#nio"><span>NIO</span></a></h2>
...
<p>NIO中的N可以理解为Non-blocking，不单纯是New。它支持面向缓冲的，基于通道的I/O操作方法。 NIO提供了与传统BIO模型中的 <code v-pre>Socket</code> 和 <code v-pre>ServerSocket</code> 相对应的 <code v-pre>SocketChannel</code> 和 <code v-pre>ServerSocketChannel</code> 两种不同的套接字通道实现,两种通道都支持阻塞和非阻塞两种模式。阻塞模式使用就像传统中的支持一样，比较简单，但是性能和可靠性都不好；非阻塞模式正好与之相反。对于低负载、低并发的应用程序，可以使用同步阻塞I/O来提升开发速率和更好的维护性；对于高负载、高并发的（网络）应用，应使用 NIO 的非阻塞模式来开发。</p>
<h3 id="nio的特性-nio与io区别" tabindex="-1"><a class="header-anchor" href="#nio的特性-nio与io区别"><span>NIO的特性/NIO与IO区别</span></a></h3>
...
<h4 id="_1-non-blocking-io-非阻塞io" tabindex="-1"><a class="header-anchor" href="#_1-non-blocking-io-非阻塞io"><span>1)Non-blocking IO（非阻塞IO）</span></a></h4>
...
<p>Java IO的各种流是阻塞的。这意味着，当一个线程调用 <code v-pre>read()</code> 或 <code v-pre>write()</code> 时，该线程被阻塞，直到有一些数据被读取，或数据完全写入。该线程在此期间不能再干任何事情了</p>
<h4 id="_2-buffer-缓冲区" tabindex="-1"><a class="header-anchor" href="#_2-buffer-缓冲区"><span>2)Buffer(缓冲区)</span></a></h4>
...
<h4 id="_3-channel-通道" tabindex="-1"><a class="header-anchor" href="#_3-channel-通道"><span>3)Channel (通道)</span></a></h4>
...
<h3 id="aio-asynchronous-i-o" tabindex="-1"><a class="header-anchor" href="#aio-asynchronous-i-o"><span>AIO (Asynchronous I/O)</span></a></h3>
...
<p>AIO 是异步IO的缩写，虽然 NIO 在网络操作中，提供了非阻塞的方法，但是 NIO 的 IO 行为还是同步的。对于 NIO 来说，我们的业务线程是在 IO 操作准备好时，得到通知，接着就由这个线程自行进行 IO 操作，IO操作本身是同步的。（除了 AIO 其他的 IO 类型都是同步的，这一点可以从底层IO线程模型解释，推荐一篇文章：<a href="https://mp.weixin.qq.com/s?__biz=Mzg3MjA4MTExMw==&amp;mid=2247484746&amp;idx=1&amp;sn=c0a7f9129d780786cabfcac0a8aa6bb7&amp;source=41#wechat_redirect" target="_blank" rel="noopener noreferrer">《漫话：如何给女朋友解释什么是Linux的五种IO模型？》</a> ）</p>
</div></template>
